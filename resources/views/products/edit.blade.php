...
<div class="mt-4">
    ...
    <x-input-error :messages="$errors->get('stock')" class="mt-2" />
                        ...
                        <!-- Photo -->
                        <div class="mt-4">
                            <x-input-label for="photo" :value="__('Foto Produk (Kosongkan jika tidak ingin diubah)')" />
                            <input type="file" id="photo" name="photo" class="block mt-1 w-full">
                            <x-input-error :messages="$errors->get('photo')" class="mt-2" />
                            @if($product->photo)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $product->photo) }}" alt="{{ $product->name }}" class="w-32 h-32 object-cover">
                                </div>
                            @endif
                        </div>
